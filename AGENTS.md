...
This project develops a local Windows caretaker for Goliath. Work only within this project folder unless the user explicitly expands scope. Do not inspect unrelated repositories, disks, private locations, or network services without that scope expansion. The owner expanded the on-demand chat's scope on 2026-09-24 to allow local system reads and requested changes; this does not authorize unrelated changes during project development.
...
The read-only direct Codex auth probe may start one short-lived app-server child that it owns, with verified PID, creation time, executable path, and exit. It must not attach to or alter the bridge-owned child, copy credentials, open a listener, or leave a resident supervisor. On Windows, chat and the auth probe resolve Codex as: optional override (`CARETAKER_CODEX_EXE` / `-CodexPath`), then the npm PATH `codex` shim's vendor `codex.exe`, then PATH `codex.exe`. They spawn the resolved `.exe` over stdio; they do not stdio-spawn the `.cmd` shim.
The owner also requested an on-demand chat window. `scripts/chat-server.js` may bind one ephemeral `127.0.0.1` port while its UI is in use and start its own native Codex app-server stdio child. It must reuse Codex's existing ChatGPT sign-in without copying credentials, disable inherited integrations before turns, and close its exact child and listener on Stop/page close/idle expiry. The model picker uses the app-server's available-model list. The owner later expanded chat beyond the five bounded read-only Caretaker actions: chat may use Codex's built-in local tools to read and request changes throughout the system. Run those turns with approval prompts for each command or file change, show the proposed action in the local UI, and execute only after the user explicitly allows that exact request. An approval is not blanket authority for unrelated actions. Chat inherits the launching Windows account's permissions; it must not self-elevate. Keep the listener ephemeral and loopback-only and do not add a persistent service, background supervisor, or another listener.
Default chat to available `gpt-6-luna` with `xhigh` reasoning, show the supported reasoning choices for the selected model, and pass the chosen effort to each turn. Fall back to an available model/effort when Luna or `xhigh` is unavailable.
The owner reported on 2026-09-24 that per-command chat approvals are too disruptive and asked for confirmation rules such as confirming deletes. The exact per-request approval flow above describes the current implementation, not the desired final experience. The next change needs a tested, mechanically bounded risk policy: low-friction diagnostics, exact confirmation for destructive/high-impact actions, no self-elevation, and no blanket trust for a full-access turn. Record the design and acceptance in `DASHBOARD_FEEDBACK.md` before enabling a different policy.
...
The owner wants a record of every chat turn and action to troubleshoot slow or failed sessions. A future bounded operational log should cover phases, duration, checks, coverage, error class, approval outcome, child lifecycle, and evidence references. A private bounded question/answer history may live under ignored `evidence/`; keep raw command arguments, file contents, credentials, and private event text out of routine logs and Git. Define redaction, retention, and user review/removal before implementation.
...
Use the self-logging setup script's read-only Plan action before setup. Its Install, Pause, and Uninstall actions may manage only receipt-verified caretaker resources covered by the user's standing approval, and must preserve the documented rollback and evidence. Rebind is a separate guarded action for a receipt-matched task pointing to a missing old checkout; a current-root identity mismatch still requires specific approval before running it. Do not enable notification delivery or create other persistent components outside that exact scope. Report project build/fixture checks separately from setup, activation, and real-caller acceptance. Separate short pilot measurements from sustained targets; do not claim 24-hour resource or retention targets achieved without representative evidence.
