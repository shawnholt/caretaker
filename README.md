...
Invoke-Item .\evidence\dashboard.html
```

The first command takes a fresh on-demand inventory; the second turns that saved evidence into a local HTML file. The page is a point-in-time report, so run the commands again to refresh it. It does not start a background server. For an on-demand AI conversation, run `node .\scripts\chat-server.js` separately and open the loopback URL it prints. Use `powershell -NoProfile -File .\scripts\caretaker.ps1 status` for the current checker and coverage state.
...
**Dashboard direction.** The dashboard is an on-demand saved-snapshot HTML report. It now addresses the feedback on local time, concise freshness, accessible help, collection drilldowns, listener review labels, and an honest memory-at-capture label. Chat remains a separate on-demand local page. Period-based resource tabs require real history and coverage; refresh, filters, and integrated chat remain future work. No resident UI service or desktop shell has been adopted. Measure normal-use UI cost before expanding it.
...
**Next implementation order.** Continue dashboard feedback items that need UI work, decide which additional Caretaker evidence an integrated UI should expose, and keep measuring normal-use cost. Keep the existing 15-minute checker and PerfMon-off setting. Do not attach to or change the bridge-owned Codex child.
