...
- `chat-server.js` prints a loopback URL for a separate on-demand chat page. Open that URL, choose a model, and ask a question. Use **Stop chat** or close the tab when finished. Node.js and a native `codex.exe` are required today (PATH `codex.exe` or `CARETAKER_CODEX_EXE`); the npm `codex` wrapper alone is not enough yet — see the Codex resolution note below. No package install, bridge, or separate API key is needed. The server binds only `127.0.0.1` on a random port, starts its own Codex app-server child when the page loads its model list, and exits on explicit Stop, Ctrl-C, 15 minutes of inactivity, or about 10 seconds after the page is hidden without being reloaded. On exit it waits up to 5 seconds for its own Codex child to exit and reports failure instead of force-killing. Chat may run the fixed read-only Caretaker checks described below when a question needs fresh evidence; it does not change the scheduled checker.
- `codex-auth-probe.ps1` starts one exact native Codex app-server child over stdio, checks `account/read` without refreshing credentials, and verifies that child exits. It prints only auth type and whether OpenAI auth is required. Pass `-CodexPath` to select a native build; without it the script uses the first `codex.exe` on PATH (not the npm `codex` shim). It creates no thread, turn, listener, or login.
...
**Verification on 2026-09-23 EDT.** Auth probe and `tests/chat-live-smoke.js` passed when pointed at a native build: `codex-cli 0.155.0-alpha.16.3` (`C:\Users\pub\.codex\plugins\.plugin-appserver\codex.exe`), `authType=chatgpt`, `requiresOpenaiAuth=true`. The page listed seven models. One turn on `gpt-6-astra` took a fresh Caretaker CPU/memory sample and reported overall CPU, RAM use, a top CPU process, sample time, and coverage. A second turn on `gpt-6-luna` took a named-process sample for `WorldOfWarshipsLegends.exe` (absent that night → UNKNOWN). An earlier same-day visual check of the page at a desktop viewport with the model picker loaded still stands. These are short functional pilots; responsive behavior, normal-use overhead, and sustained lifecycle behavior remain unmeasured.

**Codex resolution (observed 2026-09-23 / rechecked late EDT).** On this machine, `where codex` resolves to the npm shim (`codex.cmd` / `codex.ps1` → `@openai/codex`), which reported `codex-cli 0.153.4` and rejects `--no-daemon` (`unexpected argument '--no-daemon'`). Chat and the auth probe currently require a native `codex.exe`: they spawn `CARETAKER_CODEX_EXE` or PATH `codex.exe`, refuse non-`.exe` wrappers, and run `codex --no-daemon --disable apps --disable plugins mcp list --json` before `app-server`. Without a native binary on PATH, live smoke fails with HTTP 503 (`Could not verify the Codex tool configuration`). Also present locally: an older `C:\Users\pub\AppData\Local\OpenAI\Codex\bin\codex.exe` at `0.129.0-alpha.15` that likewise rejects `--no-daemon`. **Deferred fix:** teach chat and the auth probe to use the normal PATH `codex` wrapper (or resolve wrapper → current native binary reliably) so ordinary installs work without `CARETAKER_CODEX_EXE` or a hard-coded `.exe` path; keep verifying MCP disable + app-server startup against the active CLI flags for that build. Until then, set `CARETAKER_CODEX_EXE` (or put a current native `codex.exe` first on PATH) for chat/live smoke.

**Next implementation order.** Fix Codex discovery so chat/auth-probe work with the PATH wrapper (above). Continue dashboard feedback items that need UI work, decide which additional Caretaker evidence an integrated UI should expose, and keep measuring normal-use cost. Keep the existing 15-minute checker and PerfMon-off setting. Do not attach to or change the bridge-owned Codex child.
