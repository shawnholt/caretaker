...
- `chat-server.js` prints a loopback URL for a separate on-demand chat page. Open that URL, choose a model, and ask a question. Use **Stop chat** or close the tab when finished. Node.js and a native `codex.exe` on PATH are required; no package install, bridge, or separate API key is needed. The server binds only `127.0.0.1` on a random port, starts its own Codex app-server child when the page loads its model list, and exits on page close, explicit Stop, Ctrl-C, or 15 minutes of inactivity. Chat may run the fixed read-only Caretaker checks described below when a question needs fresh evidence; it does not change the scheduled checker.
