...
- `chat-server.js` prints a loopback URL for a separate on-demand chat page. Open that URL, ask a question, and use **Stop chat** or close the tab when finished. Node.js and a native `codex.exe` on PATH are required; no package install, bridge, or separate API key is needed. The server binds only `127.0.0.1` on a random port, starts its own Codex app-server child on the first question, and exits on page close, explicit Stop, Ctrl-C, or 15 minutes of inactivity. It does not change the scheduled checker or start live diagnostics.
...
**Chat behavior and limits.** The chat page uses the [Codex app-server](https://learn.chatgpt.com/docs/app-server) stdio protocol and the user's existing ChatGPT-managed Codex sign-in. Before a turn, it disables inherited app/plugin integrations and configured MCP servers, then verifies that they are disabled for the chat thread. Turns request read-only sandboxing and no approvals. Each question includes a size-bounded summary from `evidence/snapshot.json`: capture time, module coverage, a few top working-set processes, and a few TCP listener records with bind classes rather than raw addresses. Missing or partial evidence stays `UNKNOWN`; the chat has no live CPU measurement or Caretaker mutation tool. Its answer is AI interpretation of that bounded historical evidence, not a deterministic health verdict. The page holds up to 12 turns in one Codex thread, and ordinary Codex account usage limits apply.

**Verification on 2026-09-23 EDT.** The current native CLI reported `codex-cli 0.155.0-alpha.16.3`; the read-only auth probe returned `authType=chatgpt` and `requiresOpenaiAuth=true`. A two-turn direct conversation retained context. The local HTTP page returned 200, a chat POST returned a real Codex answer correctly saying live CPU was `UNKNOWN`, and Stop closed the page server with a verified child exit. The browser layout itself was not visually inspected in this pass. These checks are a short functional pilot; normal-use overhead and sustained lifecycle behavior remain unmeasured.

**Next implementation order.** Review the captured dashboard feedback, add the workload review/observation command, and decide which additional Caretaker evidence a later integrated UI should expose. Keep the existing 15-minute checker and PerfMon-off setting while measuring normal-use cost. Do not attach to or change the bridge-owned Codex child.
