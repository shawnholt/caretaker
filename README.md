...
Open the printed `http://127.0.0.1:<port>/` URL. The chat defaults to **GPT-6-Luna with Extra high reasoning** when the signed-in Codex catalog offers it; Model and Reasoning menus are beside each other. Ask, for example, “Do you see anything that could be slowing the system down right now?” The chat takes a fresh short CPU/memory sample and recent event-health check for slowdown questions, shows which checks completed, and keeps the saved snapshot clearly labeled as historical context. You can also ask it to read local files or make system changes. It shows each proposed command or file change and waits for **Allow once** or **Deny**; it runs with the Windows permissions of the account that launched the server. Use **Stop chat** when finished. A short sample can identify current load but cannot prove a sustained cause; disk latency, paging, and longer history are not measured by this question. Codex and the existing ChatGPT sign-in are required. For the static point-in-time report, run `powershell -NoProfile -File .\scripts\caretaker.ps1 snapshot`, then `powershell -NoProfile -File .\scripts\dashboard.ps1`, then open `evidence\dashboard.html`.

**Current dashboard split:** the chat page does not yet show the report's alert/change summary, inventory drilldowns, working-set table, or TCP listener detail. Those remain in the generated HTML report. Event health is a focused on-demand query, not a scheduled warning/error digest. The per-answer `OK`/`DEGRADED`/`UNKNOWN` labels describe check coverage or availability, not an overall health verdict. The owner reported slow chat and confusing check labels on 2026-09-24; the prioritized fixes and acceptance checks are in [DASHBOARD_FEEDBACK.md](DASHBOARD_FEEDBACK.md#chat-first-review--2026-09-24-edt).

## Last verified operating state (2026-09-24 EDT)

The canonical manifest is [`config/caretaker.json`](config/caretaker.json). On 2026-09-24 EDT, the enabled `GoliathCaretaker-R1` task and its receipt initially pointed to a missing older checkout; a scheduled run failed with `0x8007010B`. After a guarded `Rebind` to this checkout, setup Plan reported `Ready` with no blockers and doctor reported `READY`. An on-demand Task Scheduler run at 00:36:48 EDT and the next automatic run at 00:45:30 EDT each returned `0`; the automatic run wrote a successful snapshot with all six inventory modules collected. At that check, the following run was scheduled for 01:00:29 EDT; this is historical evidence, not a live task status. PerfMon was off and the named collector was absent; notifications were local-only with delivery disabled. The one-snapshot pilot measured 5.25 seconds wall time and 3.14 seconds process CPU on 20 logical processors, projecting to 0.017% whole-machine CPU at a 15-minute cadence. Peak memory and 24-hour overhead remain unmeasured; this is a task-only pilot, not sustained acceptance.
...
powershell -NoProfile -File .\scripts\caretaker.ps1 review
powershell -NoProfile -File .\scripts\caretaker.ps1 review set -AlertId <alert-id> -Class optional -Purpose 'why it exists' -Project 'app-or-repo'
powershell -NoProfile -File .\scripts\caretaker.ps1 review snooze -AlertId <alert-id> -Days 7
...
- `review` is the only writer of personal decisions into `config/caretaker.json`. Subcommands are `list` (default), `show`, `set`, `snooze`, and `clear`. Classes are `wanted`, `optional`, `temporary`, `review`, and `unknown`. Snooze keeps Needs attention / open alerts visible and only suppresses outbox re-nag while `snoozeUntilUtc` is in the future. Decisions stay in the manifest; evidence snapshots omit them.
...
- `dashboard.ps1` writes `evidence/dashboard.html` from the saved snapshot. It shows local capture time, freshness, collection coverage with saved-record links, point-in-time working-set memory, and a small TCP listener sample with an evidence-backed review label. It does not poll Windows or show live CPU. Regenerate it after a snapshot when needed.
- `chat-server.js` is the primary on-demand chat dashboard. It prints a loopback URL, displays the question composer and saved evidence coverage, and shows fresh Caretaker check metadata beneath each answer. Broad slowdown questions automatically run the fixed read-only resource and event-health checks before the model answers. For general requests, the Codex child has full local tools and asks for an explicit in-page approval before each command or file change. Choose a model and use **Stop chat** or close the tab when finished. Node.js and Codex are required. On Windows, resolution order is: `CARETAKER_CODEX_EXE` (optional override), the npm PATH `codex` shim's vendor `codex.exe`, then PATH `codex.exe`. The `.cmd` shim is not stdio-spawned directly. No package install, bridge, or separate API key is needed. The server binds only `127.0.0.1` on a random port, starts its own Codex app-server child when the page loads its model list, and exits on explicit Stop, Ctrl-C, 15 minutes of inactivity, or about 10 seconds after the page is hidden without being reloaded. On exit it waits up to 5 seconds for its own Codex child to exit and reports failure instead of force-killing. It does not change the scheduled checker unless you ask it to.
- `codex-auth-probe.ps1` starts one exact native Codex app-server child over stdio, checks `account/read` without refreshing credentials, and verifies that child exits. It prints only auth type and whether OpenAI auth is required. Pass `-CodexPath` to override; without it it uses the same wrapper-then-PATH resolution as chat. It creates no thread, turn, listener, or login.
...
- `setup.ps1 -Action Plan` is read-only. It checks only the exact manifest task and current readiness gates, without querying a PerfMon collector. It reported `Ready` after the 2026-09-24 rebind. Run Plan before any future setup action.
...
The manifest is the single source for task identity, cadence, and capture limits. The intended recurring checker runs every 15 minutes and uses `tick`; PerfMon collection stays off. Evidence policy targets 30 days within 1024 MiB; captures default to 60 seconds with a 300-second maximum. The implemented retention sweep applies only to valid old rows in `changes.jsonl` and reports overall budget use; protected or raw evidence can keep total storage above the target. One automatic scheduled tick proves that the repaired path ran once; it does not establish sustained operation, rollover, retention compliance, or 24-hour overhead. Do not treat resource or retention targets as achieved until representative measurement verifies them.
...
`Plan` checks the exact manifest task and setup readiness without changing system state; it does not query PerfMon or sample counters. For a moved checkout, it separately reports whether an exact receipt-matched native task is eligible for `Rebind`. `Rebind` is a guarded action that changes only that task's checker path, preserves a rollback record under ignored `evidence/`, and verifies the task and receipt afterward. The 2026-09-24 rebind was specifically authorized and verified; Plan now reports the task as owned and `Rebind` as not needed. Under [AGENTS.md](AGENTS.md), any future current-root identity mismatch still requires specific approval before another rebind.

`Install` creates one 15-minute Task Scheduler checker running `caretaker.ps1 tick` with overlap prevention and a bounded execution time. PerfMon remains off; Install creates no collector. It proceeds only when Plan's gates pass and receipt-backed identity checks permit the exact task. It never adopts or overwrites an existing task.
...
| OTHER SYSTEM CHANGES | Other tasks, collectors, services, processes, firewall/network/remote-access/security settings, drivers, OS repair, or broad cleanup. | Development automation requires a specific user request. On-demand chat can propose these actions when asked, then requires an exact in-page approval for each command or file change. Never infer ownership from a name or repeated observation. |
...
**Dashboard direction.** The on-demand chat page is the primary interaction. It puts questions first, shows per-answer fresh check metadata, and shows historical snapshot coverage alongside the conversation. The separately generated HTML report remains a point-in-time drilldown. Period-based resource trends require real history and coverage. No resident UI service or desktop shell has been adopted. Measure normal-use UI cost before expanding it.
...
The owner's first review of the generated dashboard is captured in [`DASHBOARD_FEEDBACK.md`](DASHBOARD_FEEDBACK.md). It records all eight comments and the open design questions. The current UI implements the source-backed point-in-time items; it does not add a collector or interval resource claims.

**Workload memory choice.** Stay with JSON/files. Personal review and snooze decisions live in `config/caretaker.json` under `decisions.items`; `caretaker.ps1 review` is the only writer. Observed first/last-seen data stay under ignored `evidence/`, and changes stay in the existing journal. Review classes are `wanted`, `optional`, `temporary`, `review`, and `unknown`. Snooze does not hide Needs attention; it only suppresses outbox re-nag. Only an explicit user decision changes approval rules in `desired`; observation and project guesses do not. Reassess SQLite when history joins, concurrent writers, or UI filters make file scans or atomic JSON writes materially awkward. Do not create another desired-state authority.

**Chat behavior and limits.** The chat page uses the [Codex app-server](https://learn.chatgpt.com/docs/app-server) stdio protocol and the user's existing ChatGPT-managed Codex sign-in. The picker lists models and each model's supported reasoning efforts from that signed-in app-server; both selections apply to each turn in the same conversation. Luna with Extra high is preferred when available. Before a turn, chat disables inherited app/plugin integrations and configured MCP servers, then verifies they are disabled for the chat thread. Codex turns use full local access with the `untrusted` approval policy. The local UI shows each pending command or reviewable file-change request and waits for the user to allow it once or deny it; a file patch with no supplied diff is declined, and unknown request types fail closed. Approved actions execute with the launching Windows account's rights, with no self-elevation. Caretaker also offers five structured read-only actions: fresh CPU/memory sample (with optional name search), one process by PID, focused recent event health, fresh bounded inventory by section, and Caretaker status. Their arguments are validated. Broad slowdown questions prefetch resources and recent event health even if the model makes no tool call. These actions invoke the existing self-logging project CLI, and inventory writes the normal ignored snapshot. A size-bounded saved snapshot summary also accompanies each question as **historical context**, never as proof of current state. Chat responses are AI interpretations and must identify sample time and coverage; unavailable evidence remains `UNKNOWN`. The page holds up to 12 turns in one Codex thread, and ordinary Codex account usage limits apply.

**Verification on 2026-09-23 EDT.** Auth probe and `tests/chat-live-smoke.js` passed against current Codex. After `codex update`, PATH `codex` reported `codex-cli 0.156.1` and accepts `--no-daemon`. Chat resolves that shim to its vendor `codex.exe` (same binary the wrapper runs). Earlier the same day, an explicit plugin-appserver `0.155.0-alpha.16.3` path also worked. Auth: `authType=chatgpt`, `requiresOpenaiAuth=true`. The page listed seven models. One turn on `gpt-6-astra` took a fresh Caretaker CPU/memory sample; a second on `gpt-6-luna` sampled `WorldOfWarshipsLegends.exe` (absent → UNKNOWN). An earlier visual check of the page at a desktop viewport still stands. These are short functional pilots; responsive behavior, normal-use overhead, and sustained lifecycle remain unmeasured.

**Chat-first pilot on 2026-09-24 EDT.** `node tests/chat-live-smoke.js --slowdown` asked whether anything could be slowing the system down. The reply included completed fresh resource and recent event checks captured at 00:55:36–41 EDT, with per-check `DEGRADED` coverage shown in response metadata. The answer distinguished the short CPU sample from sustained load and identified disk latency, paging, and history as unknown. This validates one live question, not continuous diagnostic accuracy or a 24-hour resource target.

**Full-access chat pilot on 2026-09-24 EDT.** In the browser, a Luna turn requested a reversible file under ignored `evidence/`. The UI displayed two read commands and the exact write/read-back command separately; each waited for **Allow once**. The resulting file contained exactly `approved` (8 bytes), and it was removed after verification. A second turn read `C:\Windows\win.ini` outside the project after a separate in-page approval and reported `[fonts]` as the first section heading. **Stop chat** exited the owned server with code 0. A separate patch-tool attempt supplied no diff, so its approval was denied and no file appeared; the server now declines patch requests with no preview. These examples prove command approval and local reach, while patch approval is only unit-tested with a supplied diff. They do not prove that every Windows operation is possible or that admin-only actions can run from a standard account.

**Codex resolution on Windows.** `where codex` finds the npm shim under `%AppData%\npm` (`codex.cmd` / `codex.ps1` → `@openai/codex`). Node cannot reliably stdio-spawn `.cmd` (`ENOENT`/`EINVAL`); spawning through `cmd.exe /c` is a poor fit for a long-lived app-server. Chat and the auth probe resolve in order: optional override (`CARETAKER_CODEX_EXE` / `-CodexPath`), the npm shim's platform vendor `codex.exe` (`@openai/codex-win32-x64`), then PATH `codex.exe`. They spawn that `.exe` with `shell: false`. Do not prefer the stale `Local\OpenAI\Codex\bin\codex.exe` (observed `0.129`, rejects `--no-daemon`) over the updated npm vendor binary. No PATH change is required when `%AppData%\Roaming\npm` is already present (as on this machine).

**Next implementation order.** Follow the [chat-first backlog](DASHBOARD_FEEDBACK.md#next-implementation-order-and-done-proof): first time the chat phases and measure system cost; then bring source-linked findings into the chat page, explain checks in plain English, replace per-command approval friction with tested risk rules, add a bounded event digest, and persist a private bounded troubleshooting trail and small file-based memory. These are planned changes, not current features. Collect disk latency, paging, and resource history only when a measured diagnosis need and bounded source justify them. Keep the existing 15-minute checker and PerfMon-off setting until a specific change is validated. Do not attach to or change the bridge-owned Codex child.
